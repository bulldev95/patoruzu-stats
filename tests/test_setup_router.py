...
class TestUploadPDF:
    ...
    def test_upload_shows_player_surnames(self, client):
        with patch("routers.setup.parse_team_sheet", return_value=PARSED_DATA):
            response = client.post(
                "/setup/upload",
                files={"file": ("teams.pdf", b"%PDF fake content", "application/pdf")},
    ...
    def test_upload_invalid_pdf_returns_200_with_error(self, client):
        response = client.post(
            "/setup/upload",
            files={"file": ("bad.pdf", INVALID_PDF_BYTES, "application/pdf")},
        )
        assert response.status_code == 200
        assert "Invalid PDF format" in response.text

    def test_upload_invalid_pdf_shows_upload_form(self, client):
        response = client.post(
            "/setup/upload",
            files={"file": ("bad.pdf", INVALID_PDF_BYTES, "application/pdf")},
        )
...
class TestConfirmSetup:
    ...
    def test_confirm_players_have_surname_and_name(self, client, db):
        client.post("/setup/confirm", data=confirm_form_data())
        player = db.query(Player).filter_by(personal_id=SAMPLE_PLAYERS[0]["personal_id"]).first()
        assert player.surname == SAMPLE_PLAYERS[0]["surname"]
    ...
    def test_confirm_existing_player_not_duplicated(self, client, db):
        existing = Player(personal_id=SAMPLE_PLAYERS[0]["personal_id"], surname="Old", name="Name")
        db.add(existing)
        db.commit()
    ...
    def test_confirm_existing_player_updated(self, client, db):
        existing = Player(personal_id=SAMPLE_PLAYERS[0]["personal_id"], surname="Old", name="Name")
        db.add(existing)
        db.commit()
